...
describe('handleConnectionDetails', () => {
  ...
  test('400 якщо roomName порожній', async () => {
    const req = new Request('http://localhost/api/connection-details?roomName=&participantName=Alice')
    const res = await handleConnectionDetails(req)
    expect(res.status).toBe(400)
  })
  ...
  test('200 і JSON з serverUrl, roomName, participantToken, participantName при валідних параметрах', async () => {
    const req = new Request('http://localhost/api/connection-details?roomName=test-room&participantName=Bob')
    const res = await handleConnectionDetails(req)
  ...
  test('500 якщо LIVEKIT_URL не задано', async () => {
    ...
    try {
      const req = new Request('http://localhost/api/connection-details?roomName=r&participantName=p')
      const res = await handleConnectionDetails(req)
