...
- **Головна** (`/`) — вибір вкладки Demo або Custom, опція E2EE
- **Кімната** (`/rooms/[roomName]`) — відеоконференція з пре-джойном та отриманням connection details через API
- **Кастомне підключення** (`/custom`) — вхід за URL сервера та JWT-токеном (з опційним E2EE)
